...
RUN_MAX = int(os.environ.get("MAX_RUN_AGE_DAYS", "4"))
DATA_MAX = int(os.environ.get("MAX_DATA_AGE_DAYS", "14"))
NOW = dt.datetime.now(dt.timezone.utc)


def age_days(stamp):
    # The journal's own stamp comes from JS toISOString(), which carries
    # milliseconds. Tolerate them rather than making the producer round-trip.
    stamp = re.sub(r"\.\d+Z$", "Z", stamp.strip())
...
if run_age is None or run_age > RUN_MAX:
    state, why = "stale", (
        "the nightly sync has not run" if run_age is None
        else f"the nightly sync last ran {run_age} days ago")
elif data_age > DATA_MAX:
    state, why = "stale", (
        f"the job is running (last run {run_age}d ago) but no new fills have been "
        f"published in {data_age} days — check whether new Webull exports are being filed")
else:
    state, why = "fresh", f"job ran {run_age}d ago, data {data_age}d old"

print(f"generated={data['generatedUtc']}")
print(f"week={data['snapshotAt']}")
